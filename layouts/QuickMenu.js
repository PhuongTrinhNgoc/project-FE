...
const QuickMenu = () => {
    ...
    useEffect(() => {
        ...
        if (infoUserCookie) {
            const userInfo = JSON.parse(infoUserCookie);
            setUserInfo(userInfo);
        }
    }, []);

    // Cập nhật cookie khi userInfo thay đổi
    useEffect(() => {
        if (userInfo) {
            Cookies.set('infoUser', JSON.stringify(userInfo), { expires: 7 });
        }
    }, [userInfo]);
            ...
            <Dropdown as="li" className="ms-2">
                <Dropdown.Toggle
                    ...
                    <div className="avatar avatar-md avatar-indicators avatar-online">
                        <Image alt="avatar" src={userInfo?.profilePicture} className="rounded-circle" />
                    </div>
                </Dropdown.Toggle>
                <Dropdown.Menu
